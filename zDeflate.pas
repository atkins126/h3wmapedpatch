...
{  Initializes the internal stream state for compression. The fields
   ...
   Z_VERSION_ERROR if the ZLibSpec library version (ZLibSpec_version) is incompatible
   with the version assumed by the caller (ZLibSpec_VERSION).
...
{ If you use the ZLibSpec library in a product, an acknowledgment is welcome
...
{$endif}
  ...
  if (windowBits < 0) then { undocumented feature: suppress ZLibSpec header }
  begin
    noheader := 1;
...
function deflateInit(var strm : z_stream; level : int) : int;
{ deflateInit is a macro to allow checking the ZLibSpec version
...
begin
  ...
  { Write the ZLibSpec trailer (adler32) }
  putShortMSB(s^, uInt(strm.adler shr 16));
  putShortMSB(s^, uInt(strm.adler and $ffff));
  flush_pending(strm);
...
{ ===========================================================================
  Initialize the "longest match" routines for a new ZLibSpec stream }
     ...
     begin
       ...
       begin
         ...
         { Slide the hash table (could be avoided with 32 bit values
           ...
           ZLibSpec, so we don't care about this pathological case.) }
