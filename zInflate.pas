...
{  inflate.c -- ZLibSpec interface to inflate modules
...
uses
  zutil, ZLibSpec, infblock, infutil;
...
{    Initializes the internal stream state for decompression. The fields
   ...
   enough memory, Z_VERSION_ERROR if the ZLibSpec library version is incompatible
                       ...
                       stream_size : int) : int;
{
   ...
   does not perform any decompression apart from reading the ZLibSpec header if
                 ...
                 f : int) : int;
{
  ...
  corrupted (input stream not conforming to the ZLibSpec format or incorrect
...
begin
  ...
  { handle undocumented nowrap option (no ZLibSpec header or check) }
...
function inflateInit(var z : z_stream) : int;
{ inflateInit is a macro to allow checking the ZLibSpec version
  ...
  case (z.state^.mode) of
    ...
    CHECK1:
      begin
        ...
        {$IFDEF DEBUG}
        Tracev('inflate: ZLibSpec check ok');
        {$ENDIF}
    ...
    FLAG:
      begin
        ...
        {$IFDEF DEBUG}
        Tracev('inflate: ZLibSpec header ok');
        {$ENDIF}
