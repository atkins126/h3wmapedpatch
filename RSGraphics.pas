...
type
  TRSXForm = object
    ...
    procedure SetE;
    procedure SetRotate(const angle:real);
    procedure SetScale(const x,y:real);
    procedure SetTransform(O1,X1,Y1,O2,X2,Y2:TPoint);
    procedure Rotate(const angle:real);
    procedure Scale(const x,y:real);
...
procedure TRSXForm.SetRotate(const angle:real);
begin
  eM11:=cos(angle);
  eM22:=eM11;
  eM12:=sin(angle);
...
procedure TRSXForm.SetScale(const x,y:real);
begin
  eM11:=x;
  eM12:=0;
  eM21:=0;
...
procedure TRSXForm.Rotate(const angle:real);
var a:TRSXForm;
begin
  with a do
  begin
...
procedure TRSXForm.Scale(const x,y:real);
begin
  eM11:=eM11*x;
  eM12:=eM12*x;
  eM21:=eM21*y;
...
begin
  ...
  try
    ...
    with PBitmapInfoHeader(PChar(Mem.Memory) + 14)^ do
 ...
 // Some incorrectly made bitmaps are loaded by graphical editors,
 // but not loaded correctly by Delphi. This fixes such cases.
function RSLoadBitmap(Stream:TStream; Bmp:TBitmap = nil):TBitmap; overload;
const
  sz = sizeof(TBitmapInfoHeader) + 14;
var
  a: TStream;
  i: int;
begin
  ...
  try
    ...
    with PBitmapInfoHeader(pchar(TMemoryStream(a).Memory) + 14)^ do
      if (i = sz) and (biCompression = BI_RGB) then
      begin
        biSizeImage:= 0;
        a:= TRSReplaceStream.Create(Stream, a, false, true, Stream.Seek(Int64(-sz), soCurrent));
      end else
      begin
        a.Free;
        a:= Stream;
...
begin
  ...
  UpCl:=ColorToRGB(UpCl);
  UpCl:=((UpCl and $ff0000) shr 16) or ((UpCl and $0000ff) shl 16) or (UpCl and integer($ff00ff00));
  DnCl:=ColorToRGB(DnCl);
  DnCl:=((DnCl and $ff0000) shr 16) or ((DnCl and $0000ff) shl 16) or (DnCl and integer($ff00ff00));
...
begin
  ...
  LeftCl:=ColorToRGB(LeftCl);
  LeftCl:=((LeftCl and $ff0000) shr 16) or ((LeftCl and $0000ff) shl 16) or (LeftCl and integer($ff00ff00));
  RightCl:=ColorToRGB(RightCl);
  RightCl:=((RightCl and $ff0000) shr 16) or ((RightCl and $0000ff) shl 16) or (RightCl and integer($ff00ff00));
...
function RSAnyTransform32(Source:TBitmap; proc:TRSSmoothTransformProc;
           ...
           ClipRect:TRect; ColorProc:TRSTransformColorProc=nil;
           ColorData:int=0; UserData:pointer=nil):TBitmap; overload;
var w,dy,x,y,x1,y1:integer; a,s:PInt;
    xp,yp:Real; p:TRSFloatPoint;
    per, col:TRSFourIntArray;
    AllP, TransP:int;
begin
  if ClipRect.Left<0 then  ClipRect.Left:=0;
  if ClipRect.Top<0 then  ClipRect.Top:=0;
  ...
  NoCl:=ColorToRGB(NoCl);
   //Перевернем NoCl
  NoCl:=((Nocl and $ff0000) shr 16) or ((Nocl and $0000ff) shl 16) or (Nocl and integer($ff00ff00));
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          per[0]:=trunc((1-xp)*(1-yp)*256);
          per[1]:=trunc((xp)*(1-yp)*256);
          per[2]:=trunc((1-xp)*(yp)*256);
          per[3]:=trunc((xp)*(yp)*256);
